...
The Jetson reports its local IP address to a web service every 5 minutes, making it easy to find for SSH access even when it changes networks:

```bash
# Query the Jetson's current IP address
curl -s "https://mpg.commsat.org/api/lastIP?password=robotanist"

# SSH into the Jetson
ssh user@$(curl -s "https://mpg.commsat.org/api/lastIP?password=robotanist")
```
